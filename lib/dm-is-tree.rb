## patched as of http://github.com/aliem/dm-is-tree
  ...
  module Is
    module Tree
      ...
      module ClassMethods
        ...
        def roots
          options1 = { tree_options[:child_key] => nil }
          options2 = { tree_options[:child_key] => 0 }
          if tree_options[:order]
            options1 = { :order => Array(tree_options[:order]) }.merge(options)
            options2 = { :order => Array(tree_options[:order]) }.merge(options)
          end
          all(options1)|all(options2)
        end
