...
module Rack
  class AutoLocale
    ...
    def call(env)
      ...
      @app.call(env) if (  @blacklist.include?(@env['PATH_INFO']) or @env['HTTP_HOST'] =~ @host or !!( @env['HTTP_HOST'] =~ @host_blacklist ) )

      if @env['PATH_INFO'] == '/'
        return redirect(get_browser_locale)
      elsif @blacklist and @blacklist.include?(@env['PATH_INFO'])
        @app.call env
      elsif @env['PATH_INFO'].match(/^\/([a-z]{2})/i) && Cfg[:locales].include?($1)
        I18n.locale = $1.to_sym
        @app.call cleanup_env(env)
      else
        return redirect get_browser_locale, @env['PATH_INFO']
      end
    end
    ...
    def get_browser_locale
      if lang = @env['HTTP_ACCEPT_LANGUAGE']
        ...
        locale = @env['locale'] = lang.first.split("-").first
      else
        locale = @env['locale'] = Cfg.default_locale
      end
